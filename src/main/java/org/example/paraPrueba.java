...
public class paraPrueba {
    ...
    private static void redesSociales() {
    ...
    private static void vistaDeSeguimiento(Scanner sc, String usuario, int tiemRed) {
        boolean salir = false;
        while (!salir) {
            asciiSeguimiento();
            System.out.println();
    ...
    private static void mostrarTareasRegistradas() {
        ...
        if (tareasCount == 0) { System.out.println("No hay tareas registradas."); }
        else { for (int i = 0; i < tareasCount; i++)
        { System.out.println((i + 1) + ") " + tareas[i]); } }
    }

    private static void mostrarHabitos() {
    ...
    private static void marcarHabitoCompletado(Scanner sc) {
        ...
        // reemplazado: usar mostrarHabitos() directamente para evitar conflicto
        mostrarHabitos();
        System.out.println("\nEscriba el número de hábito que desee marcar como completado.");
        int num = pedirNumeroValido(sc, 1, indiceHabito);
        String seleccion = habitos[num - 1];
        if (indiceHabito < habitos.length) {
            habitos[indiceHabito] = seleccion;
            indiceHabito++;
        }
        eliminarElementoArray(habitos, num - 1, indiceHabito);
        indiceHabito--;
    ...
    private static void eliminarElementoArray(String[] arr, int index, int count) {
